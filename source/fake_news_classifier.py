...
numeric_data,textual_data,labels = get_data("FakeNewsGCN/data/electionday_tweets_clean.xlsx")


# in process
# Remove punctuation and unknown characters, convert all text to lower case
path = "FakeNewsGCN/data/electionday_tweets_clean.xlsx"
original_data = pd.read_excel(path, header=0)
...
train=pd.read_csv('FakeNewsGCN/data/train.csv')
test=pd.read_csv('FakeNewsGCN/data/test.csv')
test.info()
test['label']='t'
train.info()
from sklearn.feature_extraction.text import CountVectorizer
